...
def init_user_actions():
    ...
    def add_report_mission(call: types.CallbackQuery):
        id = re.search(r'(([a-f0-9]+-){4}([a-f0-9]+))$', call.data).group(0)
        reports = func.get_count_reports_mission(id)
    ...
    def check_location(msg: types.Message, start_msg_id, id, building_id, type_rep):
        ...
        try:
            ...
            bot.register_next_step_handler(msg_new, parse_photos_report, msg_new.message_id, coords, id, building_id, type_rep, None)
        except:
            try:
                bot.delete_message(msg.chat.id, msg.message_id)
            except:
    ...
    def parse_photos_report(msg: types.Message, start_msg_id, coords, id, building_id, type_rep, group_id):
        if msg.content_type == 'text' and (msg.text == '/start' or msg.text == 'Назад'):
            handler_start(msg)
        elif not group_id is None and msg.text == 'Закончить отправку фото' and group_id is not None:
            try:
                bot.delete_message(msg.chat.id, msg.message_id)
                bot.delete_message(msg.chat.id, start_msg_id)
            except:
            ...
            new_msg = bot.send_message(msg.chat.id,
                                       ...
                                       reply_markup=types.ReplyKeyboardRemove())
            bot.register_next_step_handler(new_msg, parse_video, new_msg.message_id, coords, id, building_id, type_rep, group_id)
            return
            ...
            bot.register_next_step_handler(msg, parse_photos_report, start_msg_id, coords, id, building_id, type_rep, group_id)
        else:
            try:
                bot.delete_message(msg.chat.id, msg.message_id)
            except:
    ...
    @bot.callback_query_handler(lambda call: re.search(r'quests_user_back_(([a-f0-9]+-){4}([a-f0-9]+))$', call.data) is not None)
    def get_missions_user(call: types.CallbackQuery):
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id - 1)
        except:
